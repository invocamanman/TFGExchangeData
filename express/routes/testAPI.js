...
router.post('/releasefake', function(req, res, next) {
    ...
    const web3= new Web3("http://127.0.0.1:7545") //metamaks
    const Exchangedata= new web3.eth.Contract ( config.Exchangedata_ABI,req.body.address)//necesito address
    let seed = Web3.utils.fromAscii("111111111");
...
async function setMrFake(address, res){

    const web3= new Web3("http://127.0.0.1:7545") //metamaks
    const Exchangedata= new web3.eth.Contract ( config.Exchangedata_ABI,address)
    
    let file= path.join(__dirname, '/../resources/fakedatafordemo.csv');
...
router.post('/id', async function(req, res, next) {
    ...
    if (!found)
        ...
        const msgParams = { data:  req.body.message } //message
...
router.post('/idfake', async function(req, res, next) {
    ...
    if (!found)
        ...
        if (consumer.toLowerCase() == signaccount.toLowerCase() ) // ????
        {
            setMrFake(req.body.addrescontract, res)
