const { bufferToHex, toBuffer, keccak256 } = require('ethereumjs-util');
const {
...
//const bytesArray = (...strings) => strings.map(s => bufferToHex(toBuffer(s)));
    ...
    let seed= 12345
      // '0x00', '0x01', '0x02', ...
      console.log(Web3.utils.fromAscii("12345"))
      let leaves = genIntSequence(10)//web3.utils.fromAscii(val)
      .map(i => Web3.utils.fromAscii("12345")) // es diferente hacer tostring16, ponerlo en hexa o ponerla normal, por queeee web3.utils.fromAscii(val)
      .map(toBuffer)
      .map((value,index) =>  {
          let hash = keccak256(Buffer.concat([toBuffer(index),value]));
          //console.log(hash)
          return hash
      })
      .map(toBuffer)
      .map(bufferToHex);
//computeRoot(web3.utils.fromAscii("12346"),10)
    console.log({leaves})
    ...
    const MerkleTrees= new web3.eth.Contract ( config.MerkleseedABI,config.MerkleseedAddress)//necesito address
    ...
    web3.eth.getTransactionCount(account, (err, txCount) => {
        ...
        if (err){
            ...
            data: MerkleTrees.methods.computeRoot(seed, 10).encodeABI()
            }
